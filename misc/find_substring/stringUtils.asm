...
.nextChar:
    ...
    jz      .endOfSubstringReached                  ; if we have, check to see if we've successfully found it 
    cmp     byte [rsi], 0                           ; check if we have reached the end of the test string
    jz      .substringNotFound                      ; 
    push    rsi
    push    rdx
    ...
    pop rsi
    jz      .charMatch                              ; if they match, jump here
    jmp     .charNotMatch                           ; otherwise, here
...
.endOfSubstringReached:
    cmp     r9, 1                                   ; if the end of the substring has been reached and r9 is true,
    jz      .substringFound                         ; the substring has been successfully found
    jmp     .substringNotFound                      ; 

.substringFound:                                     ; if substring has been found, get first 
    sub     r8, rax                                 ; relative position in string = r8-rax
    mov     rax, r8                                 ; mov position to rax
    jmp     .finished

.substringNotFound:                                 ;if substring not found, set rax to -1
    mov     rax, -1
    jmp     .finished
